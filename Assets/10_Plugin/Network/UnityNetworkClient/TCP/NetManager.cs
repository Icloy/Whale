	...
	public class NetManager : KWSingleton<NetManager>
	{
		...
		{// 전체유저에게 전송되는 패킷 처리
            ...
            else if (head.m_Class == HeadClass.SOCK_ROOM)
            {
				if (head.m_Event == HeadEvent.ROOM_DATA)
				...
				else if (head.m_Event == HeadEvent.ROOM_USER_UPDATE)
					NetworkSample.instance.Recv_ROOM_USER_UPDATE(br);
					...
					NetworkSample.instance.Recv_ROOM_UPDATE(br);
            }
		}
		...
		public void Send_ROOM_USER_UPDATE( UserSession user)
		{
			BinaryWriter bw = SBPacket.WriteStart();
			user.WriteBin(bw);
			NetInterface.instance.SendData(HeadClass.SOCK_ROOM, HeadEvent.ROOM_USER_UPDATE);
		}
		public void Send_ROOM_USER_MOVE( UserSession user)
		{
			BinaryWriter bw = SBPacket.WriteStart();
			user.WriteBin(bw);
			NetInterface.instance.SendData(HeadClass.SOCK_ROOM, HeadEvent.ROOM_USER_MOVE);
		}
	}
}
