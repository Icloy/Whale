        ...
        {// 전체유저에게 전송되는 패킷 처리
            ...
            else if (head.m_Class == HeadClass.SOCK_ROOM)
            {
                if (head.m_Event == HeadEvent.ROOM_BROADCAST)
                    MainManager.Instance.netGameManager.Recv_ROOM_BROADCAST(br);
                else if (head.m_Event == HeadEvent.ROOM_USER_DATA_UPDATE)
                    MainManager.Instance.netGameManager.Recv_ROOM_USER_DATA_UPDATE(br);
                else if (head.m_Event == HeadEvent.ROOM_USER_MOVE_DIRECT)
                    MainManager.Instance.netGameManager.Recv_ROOM_USER_MOVE_DIRECT(br);
                else if (head.m_Event == HeadEvent.ROOM_USER_ITEM_UPDATE)
                    MainManager.Instance.netGameManager.Recv_ROOM_USER_ITEM_UPDATE(br);
                else if (head.m_Event == HeadEvent.ROOM_ENTER)
                    MainManager.Instance.netGameManager.Recv_ROOM_ENTER(br);
                else if (head.m_Event == HeadEvent.ROOM_MAN_IN)
                    MainManager.Instance.netGameManager.Recv_ROOM_MAN_IN(br);
                else if (head.m_Event == HeadEvent.ROOM_MAN_OUT)
                    MainManager.Instance.netGameManager.Recv_ROOM_MAN_OUT(br);
                else if (head.m_Event == HeadEvent.ROOM_DATA_UPDATE)
                    MainManager.Instance.netGameManager.Recv_ROOM_DATA_UPDATE(br);
                else if (head.m_Event == HeadEvent.ROOM_UPDATE)
                    MainManager.Instance.netGameManager.Recv_ROOM_UPDATE(br);
            }
        }
